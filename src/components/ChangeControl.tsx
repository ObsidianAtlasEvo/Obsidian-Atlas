...
export function ChangeControl({ state, setState }: ChangeControlProps) {
  ...
  const getClassColor = (cls: number) => {
    switch (cls) {
      case 4: return 'text-crimson-900 border-crimson-900/20 bg-crimson-900/5';
      case 3: return 'text-gold-500 border-gold-500/20 bg-gold-500/5';
      case 2: return 'text-ivory border-stone-800/20 bg-stone-800/5';
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-[10px] font-mono uppercase tracking-widest text-stone">Governance & Deployment Management</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {loading ? (
          <div className="py-20 text-center text-stone uppercase tracking-widest pulse-shimmer rounded-sm">Scanning Change Queue...</div>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[8px] font-mono uppercase tracking-widest px-2 py-0.5 bg-stone-800/10 text-stone border border-stone-800/20 rounded-full">Class {proposal.class}</span>
                    </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <span className="text-[8px] font-mono text-stone uppercase tracking-widest">Created: {new Date(proposal.createdAt).toLocaleDateString()}</span>
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-stone-800/50 flex items-center justify-between">
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className={cn("text-[10px] font-mono", proposal.rollbackSafe ? 'text-teal' : 'text-crimson-900')}>
              ...
              {changeControlStore.activeMutationId === proposal.id && changeControlStore.mutationLogs.length > 0 && (
                <div className="mt-4 p-4 bg-obsidian border border-stone-800 rounded-sm font-mono text-[10px] text-stone space-y-1">
                  {changeControlStore.mutationLogs.map((log, idx) => (
                    <div key={idx} className="animate-pulse-subtle">{log}</div>
                  ))}
                </div>
              )}
            </motion.div>

...
/** Browser build: server emits via AtlasEventBus; panel runs client-only (no-op). */
...
export class UserEvolutionControl {
  ...
  private emitControlEvent(
    _userId: string,
    _type: AtlasEventType,
    _payload: Record<string, unknown>,
  ): void {
    /* Optional: POST /v1/governance/evolution-control/event in a later wiring pass */
  }
}
